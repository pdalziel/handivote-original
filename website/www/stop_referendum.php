<?php 
/* 
Stop a referendum, referendum ID supplied via GET
...
if ($row = mysql_fetch_assoc($result_referendum)) {
  ...
  /* if the referendum is "in progress" we can stop it by chanigng the status to "finished" */

  if (strcmp($row['status'], "in progress")==0){
  	

     $update_referendum = sprintf("UPDATE `referendum` SET `status`= 'finished' WHERE `id` LIKE '%s';", $id);
     $result_update = mysql_query($update_referendum, $con);

/* success message */
    print "<h4>Referendum was successfully closed.</h4></br></br>";
  }

else{

     /* if they got here, the status of the referendum is not "in progress"; therefore we cannot stop it; terminate */
    die();
